    ...
    def validate(self) -> None:
        ...
        if not sync_config:
            ...
            if not isinstance(rate_limit, int) or rate_limit <= 0:
                ...
                warnings.append("sync.rate_limit > 1000 may cause issues with Jira API limits")
        ...
        if custom_fields and not isinstance(custom_fields, list):
            ...
            for i, field in enumerate(custom_fields):
                ...
                if not field.get('id'):
                    ...
                    warnings.append(f"custom_fields[{i}].id should typically start with 'customfield_'")
                
                if not field.get('name'):
                    errors.append(f"custom_fields[{i}].name is required")
                
                field_type = field.get('type', 'text')
                if field_type not in ['text', 'number', 'select', 'multiselect', 'date']:
                    warnings.append(f"custom_fields[{i}].type '{field_type}' is not a recognized type")
    ...
    def update_custom_fields_from_jira(self, jira_client) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for field in discovered_fields:
                ...
                field_type = self._map_jira_field_type(field.get('schema', {}).get('type', 'string'))
