...
class JiraClient:
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            if e.response.status_code >= 500:
                raise JiraClientError(
                    f"Jira server error ({e.response.status_code}): {e}"
                )
            else:
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully connected as user: {user_info.get('displayName', 'Unknown')}"
            )
    ...
    def search_issues(
        ...
        logger.debug(
            f"Found {result.get('total', 0)} issues, returned {len(result.get('issues', []))}"
        )
        return result
    ...
    def get_issue(
        ...
        params = {"fields": ",".join(fields), "expand": "names"}
    ...
    def get_project_custom_fields(self, project_key: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._make_request("GET", f"project/{project_key}")
