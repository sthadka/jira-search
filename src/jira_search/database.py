...
class Database:
    ...
    def _create_issues_table(self, conn: sqlite3.Connection) -> None:
        ...
        required_columns = ["key TEXT PRIMARY KEY", "summary TEXT NOT NULL"]
        ...
        core_field_columns.extend(
            ["project_key TEXT", "project_name TEXT", "issue_type TEXT"]
        )
    ...
    def _process_issue_data(self, issue_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed = {"key": issue_data.get("key"), "summary": fields.get("summary", "")}
        ...
        for field in self.config.core_fields:
            ...
            if field == "description":
                ...
                processed["assignee_display_name"] = (
                ...
                processed["reporter_display_name"] = (
                ...
                if components:
                    component_names = [
                        comp.get("name", "") for comp in components if comp.get("name")
                    ]
                ...
                if fix_versions:
                    version_names = [
                        ver.get("name", "") for ver in fix_versions if ver.get("name")
                    ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute(
                    "SELECT key FROM issues ORDER BY created ASC LIMIT 1"
                )
                oldest_issue = cursor.fetchone()
                oldest_issue = oldest_issue[0] if oldest_issue else None

                cursor = conn.execute(
                    "SELECT key FROM issues ORDER BY created DESC LIMIT 1"
                )
    ...
    def _sanitize_fts_query(self, query: str) -> str:
        ...
        if any(char in query for char in special_chars) and not (
