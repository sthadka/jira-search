...
class AdvancedSearch:
    ...
    def search(self, query: str, mode: str, limit: int = 100, offset: int = 0) -> Tuple[List[Dict[str, Any]], int]:
        ...
        try:
            ...
            logger.error(f"Search failed (mode={mode}, query={query}, elapsed={elapsed:.3f}s): {e}")
            raise
    
    def _search_jql(self, jql_query: str, limit: int = 100, offset: int = 0) -> Tuple[List[Dict[str, Any]], int]:
        ...
        try:
            ...
            with sqlite3.connect(self.db.db_path) as conn:
                ...
                cursor = conn.execute(count_sql, params[:-2])  # Remove limit/offset params
    ...
    def _parse_operator_condition(self, condition: str, jql_op: str, sql_op: str) -> Tuple[str, List[Any]]:
            ...
            with sqlite3.connect(self.db.db_path) as conn:
                ...
                def regexp_with_timeout(pattern, text):
                    if time.time() - start_time > self.timeout_seconds:
                        raise RegexError(f"Regex search timed out after {self.timeout_seconds} seconds")
