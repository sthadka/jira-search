...
@cli.command()
@click.option(
    "--with-examples", is_flag=True, help="Include detailed examples and comments"
)
@click.option(
    "--minimal",
    is_flag=True,
    help="Generate minimal configuration with required fields only",
)
@click.pass_context
    ...
    try:
        ...
        if detailed:
            ...
            if config.sync_project_key:
                ...
                click.echo("  • Sync scope: Custom JQL query")
            ...
            try:
                ...
                if env_vars_used:
                    click.echo(
                        f"  • Environment variables: {', '.join(set(env_vars_used))}"
                    )
            except Exception:
                pass
...
@click.option(
    "--filter", help="Filter fields by name (case-insensitive substring match)"
)
@click.pass_context
def discover_fields(
    ctx, project: Optional[str], output: Optional[str], filter: Optional[str]
):
    ...
    try:
        ...
        if not connection_result["success"]:
            click.echo(
                f"✗ Jira connection failed: {connection_result['error']}", err=True
            )
            sys.exit(1)
        ...
        if filter:
            ...
            click.echo(
                f"🔍 Filtered to {len(custom_fields)} fields matching '{filter}'"
            )
        ...
        click.echo("💡 Tips:")
        click.echo(
            "  • Fields marked with ⭐ are likely useful for search and filtering"
        )
        click.echo("  • Add relevant fields to your config.yaml custom_fields section")
        click.echo(
            "  • Re-sync your data after adding custom fields: python -m jira_search sync --full"
        )
...
def sync(
    ...
    try:
        ...
        if jql:
            ...
            click.echo(
                "Error: No sync query specified. Use --project, --jql, or configure in config.yaml",
                err=True,
            )
            sys.exit(1)
...
@click.option(
    "--include-deleted", is_flag=True, help="Include deleted issues in export"
)
@click.pass_context
def export(
    ctx, format: str, output: Optional[str], query: Optional[str], include_deleted: bool
):
    ...
    try:
        ...
        # Use all issues if no query provided
        if not query:
            query = "*"
            ...
            if format.lower() == "csv":
                ...
                if issues:
                    ...
                    for issue in issues:
                        ...
                        clean_issue = {
                            k: str(v) if v is not None else "" for k, v in issue.items()
                        }
...
def cleanup(ctx, days: int, dry_run: bool):
    ...
    try:
        ...
        if dry_run:
            ...
            click.echo(
                f"DRY RUN - Would remove {count} issues deleted more than {days} days ago"
            )
            return
...
@cli.command()
@click.option(
    "--output", "-o", required=True, help="Backup file path (.db extension recommended)"
)
@click.pass_context
...
@click.option(
    "--force", is_flag=True, help="Overwrite existing database without confirmation"
)
@click.pass_context
